...
namespace AMEC.PCSoftware.CommunicationProtocol.CrazyHein.OrbmentDAQ.IOUtility
{
        ...
        public int Receive(byte[] buffer, int offset, int size, SocketFlags socketFlags = SocketFlags.None)
        {
            ...
            try
            {
                while (length != size)
                {
                    ...
                    if (length == 0)
                        throw new DAQException(DAQExceptionCode.REMOTE_SERVER_DISCONNECTED);
                }
            }
            catch (DAQException)
            {
